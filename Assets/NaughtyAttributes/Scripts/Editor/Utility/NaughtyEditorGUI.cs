...
namespace NaughtyAttributes.Editor
{
    public static class NaughtyEditorGUI
    {
        ...
        public static void GroupedButton(UnityEngine.Object target, MethodInfo methodInfo)
        {
            bool visible = ButtonUtility.IsVisible(target, methodInfo);
            if (!visible)
            {
            ...
            if (methodInfo.GetParameters().All(p => p.IsOptional))
            {
                ButtonGroupAttribute buttonAttribute = (ButtonGroupAttribute)methodInfo.GetCustomAttributes(typeof(ButtonGroupAttribute), true)[0];
                string buttonText = string.IsNullOrEmpty(buttonAttribute.Text) ? ObjectNames.NicifyVariableName(methodInfo.Name) : buttonAttribute.Text;
            ...
            {
                string warning = typeof(ButtonGroupAttribute).Name + " works only on methods with no parameters";
                HelpBox_Layout(warning, MessageType.Warning, context: target, logToConsole: true);
            }
        }
