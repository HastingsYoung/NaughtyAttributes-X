...
namespace NaughtyAttributes.Editor
{
    public static class ReflectionUtility
    {
        ...
        public static IEnumerable<Attribute> GetAllClassAttributes(object target)
        {
            List<Type> types = GetSelfAndBaseTypes(target);

            for (int i = types.Count - 1; i >= 0; i--)
            {
                IEnumerable<Attribute> classAttrs = Attribute.GetCustomAttributes(types[i], typeof(ClassAttribute));
